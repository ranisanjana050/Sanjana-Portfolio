...
export const Experience = () => {
  return (
    <div id="exp" className="bg-black py-20">
      <div className="flex flex-col justify-center items-center">
        ...
        <div className="text-white text-center mt-10 w-[90vw] max-w-xl">
          ...
          <p className="text-slate-300">
            Here are some of my experiences in the IT & CX field.
          </p>
        </div>
              ...
              >
                ...
                <ul className="list-disc list-inside text-slate-300 text-sm mb-3 space-y-1">
                  {experience.desc.map((item, i) => (
                     <div className='pt-4'>
                       <h2 key={i} className='text-md'>{i+1}. {item.sub}</h2>
                        {item.desc.map((point, j) => (
                          <li key={j} className="text-slate-400">{point}</li>
                        ))}
                     </div>
                  ))}
                </ul>

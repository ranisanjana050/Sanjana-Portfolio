         ...
         className='hover:transform hover:transition hover:duration-500 '>
            {
                    ...
                    className={project?.bg}>
                      ...
                      <img src={project?.image} alt="" className='hover:scale-105 transition-all ease-in-out duration-500 rounded-md mb-4 w-3/2 h-44 md:h-40 lg:h-48 xl:h-52 '/>
                      </a>
                     ...
                     {  project.tags.map((tag)=>(
                        ...
                        <p className='text-xs mb-2 text-slate-500 mt-1 '>{project.date}</p>
                        <p className='mb-2 text-slate-500'>
                            {project.description}
                        </p>
                        <div className='flex gap-10 mt-5'>
                        <div className='transform transition duration-500 hover:scale-150 cursor-pointer'>
                        <a href={project.github} target='_blank' rel='noreferrer'><FaGithub size={35} 
                          onMouseOut={({target})=>target.style.color="white"}
                          onMouseOver={({target})=>target.style.color="#854CE6"}
                        /></a>
                        </div>
                        <div className='transform transition duration-500 hover:scale-150 cursor-pointer'>
                        <a href={project.webapp} target='_blank' rel='noreferrer'><FaLink size={30}
                          onMouseOut={({target})=>target.style.color="white"}
                          onMouseOver={({target})=>target.style.color="#854CE6"}
                        /></a>
                        </div>
